...
ccPW = pd.read_csv('albanoRomanceCCbinPW.csv',index_col=0,dtype='str')
ccSW = pd.read_csv('albanoRomanceCCbinSW.csv',index_col=0,dtype='str')
...
with open('romanceCC_PW.tsv','w') as f:
    for i in ccPW.index:
        f.write(i+'\t')
        f.write('\t'.join(ccPW.loc[i].values)+'\n')

with open('romanceCC_SW.tsv','w') as f:
    for i in ccSW.index:
        f.write(i+'\t')
...
with open('convertRomancePosteriorPW.paup','w') as f:
    f.write(paupCommandsPW)

with open('convertRomancePosteriorSW.paup','w') as f:
