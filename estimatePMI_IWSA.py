...
Original file is located at
    https://colab.research.google.com/drive/1upYDtZybE3YCR0RoI4naIBn-1_q1FEEn
"""
...
# Looks like the actual NW alignment that has to be replaced is in here. Then what is alignment.py for?
# Some of the files aren't even fully used... I guess I should just use IWSA here instead of NW and that's it.
...
def iwsa_align(w, pmiDict, gp1, gp2, th=-Inf):
    x, y = w
...
def iwsa_align_star(crp, pmiDict, gp1, gp2, th):
    ...
    return_dict = manager.dict()
    def doWork(i, pck):
        # Wrap the computation in a try-except block to catch potential errors
        try:
            result = vstack([iwsa_align(w, pmiDict, gp1, gp2, th) for w in pck])
            return_dict[i] = result
        except Exception as e:
            print(f"Error in process {i}: {e}") # Print any errors encountered
    jobs = []
    for i, pck in enumerate(packages):
        p = Process(target=doWork, args=(i, pck))
        p.start()
    ...
    for p in jobs:
        p.join()
    # Check if return_dict is empty before stacking
    if return_dict:
        return vstack(return_dict.values())
    else:
        print("Warning: No results returned from child processes.")
        return np.array([]) # Return an empty array if no results
